...
public class AccountsController {
  ...
  public ResponseEntity<TransferResponse> transferMoney(@Valid @RequestBody TransferRequest request) {
    try {
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new TransferResponse(request.sourceAccountId(), request.destinationAccountId(), request.amount(),"An error occurred during transfer"));
    }
  }

  private void validateAccount(TransferRequest request) {
    if(request.sourceAccountId().equals(request.destinationAccountId())) {
      throw new SelfFundTransferException("Can not transfer fund to the same account");
    }
  }
